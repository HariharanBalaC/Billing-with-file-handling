...
color4='#F5C689'     #87918c
color5='#F5C689'       #87918c
color6='#F5C689'     #87918c
EMAIL_ADDRESS = os.environ.get('EMAIL_USER')  # your email address
EMAIL_PASSWORD = os.environ.get('EMAIL_PASS')  # password
x = randint(99, 200)
path = 'C:\\Users\\harih\\BILLS'  # path to the folder
root = Tk()
root.geometry("1150x750")        #+0+0
root.title("Restaurant Billing System")
#bg = PhotoImage(file = "brickwall.png")
Tops = Frame(root, width=1350, height=50, bd=8, bg=color4, relief="flat")
Tops.pack(side=TOP)
Bottoms = Frame(root, width=1350, height=50, bd=8, bg=color4, relief="flat")
Bottoms.pack(side=BOTTOM)
f1 = Frame(root, width=900, height=650, bd=8, bg=color, relief="flat")
f1.pack(side=LEFT)
f1a = Frame(f1, width=900, height=330, bd=8, bg=color, relief="flat")
f1a.pack(side=TOP)
f2a = Frame(f1, width=900, height=320, bd=8, bg=color, relief="flat")
f2a.pack(side=BOTTOM)
f1aa = Frame(f1a, width=400, height=430, bd=8, bg=color4, relief="flat")
f1aa.pack(side=LEFT)
f1ab = Frame(f1a, width=400, height=430, bd=8, bg=color5, relief="flat")
f1ab.pack(side=RIGHT)
f2aa = Frame(f2a, width=450, height=330, bd=8, bg=color6, relief="flat")
f2aa.pack(side=LEFT)
f2ab = Frame(f2a, width=450, height=330, bd=8, bg=color4, relief="flat")
f2ab.pack(side=LEFT)
lblInfo = Label(Tops, font=('arial', 25, 'bold'), text="Restaurant Billing System", bg=color4,bd=15,padx=12, anchor='w')
...
def output():
    ...
    list5 = "Naan\t\t" + naan.get() + "\t\t\t" + str(
        int(naan.get()) * int(costnaan.get())) + "\n\n"
...
# --------------
lblSd = Label(f1aa, font=('arial', 16, 'bold'), text="Naan", bd=16, bg=color4, justify='left')
lblSd.grid(row=4, column=0)
txtSd = Entry(f1aa, font=('arial', 16, 'bold'), textvariable=naan, bd=10, insertwidth=2, justify='left')
...
# --------------
lblCsd = Label(f1ab, font=('arial', 16, 'bold'), text="Price of Naan", bd=16, bg=color5, justify='left')
lblCsd.grid(row=4, column=0)
txtCsd = Entry(f1ab, font=('arial', 16, 'bold'), textvariable=costnaan, bd=10, insertwidth=2, justify='left')
...
btnTotal = Button(f2ab, padx=16, pady=16, bd=8, fg="black", font=('arial', 16, 'bold'), width=15,
                  text="Total Price", bg=color1, command=tPrice).grid(row=1, column=0)
btnRefer = Button(f2ab, padx=16, pady=16, bd=8, fg="black", font=('arial', 16, 'bold'), width=15,
                  text="Sales Reference", bg=color1, command=refNo).grid(row=0, column=0)
...
btnReset = Button(f2ab, padx=16, pady=16, bd=8, fg="black", font=('arial', 16, 'bold'), width=15,
                  text="Reset", bg=color1, command=reset).grid(row=0, column=1)
